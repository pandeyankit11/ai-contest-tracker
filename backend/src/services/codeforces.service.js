...
function assertValidProfile(profile) {
  ...
  if (!hasValidHandle) {
    throw new AppError("Codeforces returned an invalid profile response", 502, "CODEFORCES_INVALID_RESPONSE");
  }
}
...
function assertValidFetchResponse(response) {
  ...
  if (!isValidResponse) {
    throw new AppError("Codeforces returned an invalid HTTP response", 502, "CODEFORCES_INVALID_RESPONSE");
  }
}
...
function assertValidRatingUpdate(update) {
  ...
  if (!hasValidRatingUpdate) {
    throw new AppError("Codeforces returned an invalid rating history response", 502, "CODEFORCES_INVALID_RESPONSE");
  }
}
...
// --- THE FIX 1: Relaxed validation so we don't drop problems with weird API data ---
function assertValidAcceptedSubmission(submission) {
  if (!submission || typeof submission !== "object" || submission.verdict !== "OK") {
    throw new AppError("Not an accepted submission", 502, "CODEFORCES_INVALID_RESPONSE");
  }
  if (!submission.problem || typeof submission.problem !== "object") {
    throw new AppError("Missing problem details", 502, "CODEFORCES_INVALID_RESPONSE");
  }
  if (!buildProblemExternalId(submission.problem)) {
    throw new AppError("Cannot generate problem ID", 502, "CODEFORCES_INVALID_RESPONSE");
  }
}
...
function formatSolvedProblem(userId, submission) {
  ...
  // Safe fallback for date if Codeforces provides corrupted timestamps
  const solvedAt = Number.isFinite(submission.creationTimeSeconds)
  ...
  return {
    ...
    url: buildProblemUrl(problem) || `https://codeforces.com/problemset`,
    tags: normalizeTags(problem.tags),
    difficulty: null,
    
    // --- THE FIX 2: Default null ratings to 0 to prevent Prisma crashes ---
    rating: normalizeNumber(problem.rating) || 0,
    solvedAt,
  };
}
...
async function fetchCodeforcesPayload(url) {
  if (typeof fetchImplementation !== "function") {
    throw new AppError("Codeforces API client is not available", 502, "CODEFORCES_API_UNAVAILABLE");
  }
  ...
  try {
    ...
    throw new AppError("Unable to reach Codeforces API", 502, "CODEFORCES_API_UNAVAILABLE");
  }

  assertValidFetchResponse(response);

  if (response.status === 429) {
    throw new AppError("Codeforces API rate limit exceeded", 429, "CODEFORCES_RATE_LIMITED");
  }

  if (!response.ok) {
    throw new AppError("Codeforces API request failed", 502, "CODEFORCES_API_ERROR");
  }
  ...
  try {
    ...
    throw new AppError("Codeforces returned an invalid JSON response", 502, "CODEFORCES_INVALID_RESPONSE");
  }
  ...
  if (payload.status === "FAILED") {
    ...
    throw new AppError(payload.comment || "Codeforces API request failed", 502, "CODEFORCES_API_ERROR");
  }
...
async function getUserCodeforcesAccount(userId) {
  return prisma.contestAccount.findFirst({
    where: { userId, platform: CODEFORCES_PLATFORM },
    select: { handle: true },
  });
}

async function fetchCodeforcesProfile(handle) {
  const payload = await fetchCodeforcesPayload(`${CODEFORCES_USER_INFO_URL}?handles=${encodeURIComponent(handle)}`);
  const hasValidResult = payload.status === "OK" && Array.isArray(payload.result) && payload.result.length > 0;
...
async function fetchCodeforcesRatingHistory(handle) {
  const payload = await fetchCodeforcesPayload(`${CODEFORCES_RATING_HISTORY_URL}?handle=${encodeURIComponent(handle)}`);
...
async function fetchCodeforcesSubmissions(handle) {
  const payload = await fetchCodeforcesPayload(`${CODEFORCES_SUBMISSIONS_URL}?handle=${encodeURIComponent(handle)}`);
  const hasValidResult = payload.status === "OK" && Array.isArray(payload.result);

  if (!hasValidResult) {
    throw new AppError("Codeforces returned an invalid submissions response", 502, "CODEFORCES_INVALID_RESPONSE");
  }
...
async function syncRatingHistory(userId, handle) {
  ...
  const sortedHistory = [...ratingHistory].sort((left, right) => left.ratingUpdateTimeSeconds - right.ratingUpdateTimeSeconds);
  ...
  for (const update of sortedHistory) {
    try {
      assertValidRatingUpdate(update);
      maxRating = maxRating === null ? update.newRating : Math.max(maxRating, update.newRating);
...
async function syncSolvedProblems(userId, handle) {
  ...
  for (const submission of submissions) {
    ...
    try {
      ...
      // Don't fail the loop, just skip the problem
      continue;
    }
  }
  ...
  for (const solvedProblem of acceptedProblems.values()) {
    try {
      ...
      // --- THE FIX 3: Reveal the silent errors so we can actually see them on Render ---
      console.error(`[CODEFORCES SYNC] DB Error saving ${solvedProblem.externalId}:`, error.message);
      results.failed += 1;
    }
  }
...
async function syncAllUserData(userId) {
  ...
  if (!account) {
    throw new AppError("Codeforces account not found for this user", 404, "CODEFORCES_ACCOUNT_NOT_FOUND");
  }
